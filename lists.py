...
def approx_average_is_average(hand):
    ...
    n = len(hand)
    actual_average = sum(hand) / n

    # Approximation 1: Average of first and last number
    approx_average_1 = (hand[0] + hand[-1]) / 2

    # Approximation 2: Median (middle card)
    sorted_hand = sorted(hand)
    median = sorted_hand[n // 2]

    return approx_average_1 == actual_average or median == actual_average
...
def average_even_is_average_odd(hand):
    ...
    """
    if len(hand[0::2]) > 0 and len(hand[1::2]) > 0 and sum(hand[0::2]) / len(hand[0::2]) == sum(hand[1::2]) / len(hand[1::2]):
        return True
    else:
        return False
