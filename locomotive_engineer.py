...
def fix_list_of_wagons(each_wagons_id, missing_wagons):
    ...
    if not each_wagons_id:
        return [1] + missing_wagons

    first_two = []
    remaining_wagons = list(each_wagons_id)  

    if len(remaining_wagons) >= 2:
        first_two = remaining_wagons[:2]
        remaining_wagons = remaining_wagons[2:]
    elif len(remaining_wagons) == 1:
        first_two.append(remaining_wagons[0])
        remaining_wagons = []

    try:
        locomotive_index = remaining_wagons.index(1)
        modified_wagons = remaining_wagons[:locomotive_index + 1] + missing_wagons + remaining_wagons[locomotive_index + 1:] + first_two
        return modified_wagons
    except ValueError:
        return [1] + missing_wagons + remaining_wagons + first_two
...
def fix_wagon_depot(wagons_rows):
    ...
    """
    fixed_rows = [[] for _ in range(3)]
    
    
    for i in range(3):
        for j in range(3):
        
            fixed_rows[j].append(wagons_rows[i][j])
